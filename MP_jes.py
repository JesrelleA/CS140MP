...
dishlist = []
name = "read_from_file" # input
time = "read_from_file" # input
priority = "read_from_file" # input
    ...
    if dishlist[0].time == time: #checks if the dish will enter the scheduler
        ...
        if task == "cook":
            if len(cook) == 0 and len(ready) == 0:  #as in walang dish na cino-cook and nagwaiwait para macook hehe so enter na agad sa 'Cook' column
                cook[0] = current
            else: #if len(cook) == 0 and len(ready) != 0: 
        ...
        if len(cook) == 0 and len(ready) == 0: #wala ng icoo-cook
            #print in Cook: none
            print("Sept 7: add here")

        if len(cook) == 0 and len(ready) != 0:
            #INSERT ALGO THAT GETS THE DISH WITH THE HIGHEST PRIORITY
            winner = 0 # add here; gets the dish with the highest priority to be cooked
